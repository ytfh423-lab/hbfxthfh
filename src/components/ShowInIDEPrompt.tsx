...
export function ShowInIDEPrompt(t0) {
  ...
  let t2;
  if ($[2] !== symlinkTarget) {
    t2 = symlinkTarget && <Text color="warning">{relative(getCwd(), symlinkTarget).startsWith("..") ? `这将通过符号链接修改 ${symlinkTarget}（工作目录外）` : `符号链接目标：${symlinkTarget}`}</Text>;
    $[2] = symlinkTarget;
    $[3] = t2;
  } else {
    t2 = $[3];
  ...
  let t5;
  if ($[7] !== t4) {
    t5 = <Text>你要对{" "}<Text bold={true}>{t4}</Text> 进行此编辑吗？</Text>;
    $[7] = t4;
    $[8] = t5;
  } else {
    t5 = $[8];
