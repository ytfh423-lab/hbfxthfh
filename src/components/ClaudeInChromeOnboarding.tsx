...
export function ClaudeInChromeOnboarding(t0) {
  ...
  if ($[4] !== isExtensionInstalled) {
    t4 = !isExtensionInstalled && <><Newline /><Newline />需要 Chrome 扩展。前往{" "}<Link url={CHROME_EXTENSION_URL} /> 开始</>;
  ...
  let t5;
  if ($[6] !== t4) {
    t5 = <Text>Claude in Chrome 与 Chrome 扩展配合使用，让你可以直接从 Claude Code 控制浏览器。你可以浏览网站、填写表单、截取屏幕截图、录制 GIF，以及使用控制台日志和网络请求进行调试。{t4}</Text>;
    $[6] = t4;
    $[7] = t5;
  } else {
    t5 = $[7];
  ...
  let t7;
  if ($[10] !== t6) {
    t7 = <Text dimColor={true}>站点级权限继承自 Chrome 扩展。在 Chrome 扩展设置中管理权限，以控制 Claude 可以浏览、点击和输入的站点{t6}。</Text>;
    $[10] = t6;
    $[11] = t7;
  } else {
    t7 = $[11];
  ...
  if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
    t9 = <Text dimColor={true}>更多信息请使用{" "}{t8}{" "}或访问 <Link url="https://code.claude.com/docs/en/chrome" /></Text>;
    $[13] = t9;
  } else {
    t9 = $[13];
  }
  ...
  let t11;
  if ($[17] !== onDone || $[18] !== t10) {
    t11 = <Dialog title="Claude in Chrome (测试版)" onCancel={onDone} color="chromeYellow">{t10}</Dialog>;
    $[17] = onDone;
    $[18] = t10;
    $[19] = t11;
  } else {
