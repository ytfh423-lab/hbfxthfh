...
export function QuestionView(t0) {
  ...
  let t17;
  if ($[80] !== t15 || $[81] !== t16) {
    t17 = <Text color={t15}>{t16}. 聊聊这个问题</Text>;
    $[80] = t15;
    $[81] = t16;
    $[82] = t17;
  } else {
  ...
  if ($[86] !== footerIndex || $[87] !== isFooterFocused || $[88] !== isInPlanMode || $[89] !== options.length) {
    t19 = isInPlanMode && <Box flexDirection="row" gap={1}>{isFooterFocused && footerIndex === 1 ? <Text color="suggestion">{figures.pointer}</Text> : <Text> </Text>}<Text color={isFooterFocused && footerIndex === 1 ? "suggestion" : undefined}>{options.length + 2}. 跳过访谈并立即规划</Text></Box>;
    $[86] = footerIndex;
    $[87] = isFooterFocused;
    $[88] = isInPlanMode;
    $[89] = options.length;
  ...
  if ($[94] !== questions.length) {
    t21 = questions.length === 1 ? <>{figures.arrowUp}/{figures.arrowDown} 导航</> : "Tab/箭头键导航";
    $[94] = questions.length;
    $[95] = t21;
  } else {
    t21 = $[95];
  ...
  let t23;
  if ($[98] !== t21 || $[99] !== t22) {
    t23 = <Box marginTop={1}><Text color="inactive" dimColor={true}>回车选择 ·{" "}{t21}{t22}{" "}· Esc 取消</Text></Box>;
    $[98] = t21;
    $[99] = t22;
    $[100] = t23;
  } else {
