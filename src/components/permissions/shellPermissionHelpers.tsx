...
export function generateShellSuggestionsLabel(suggestions: PermissionUpdate[], shellToolName: string, commandTransform?: (command: string) => string): ReactNode | null {
  ...
  if (hasReadPaths && !hasDirectories && !hasCommands) {
    ...
    if (readPaths.length === 1) {
      ...
      return <Text>
          是的，允许读取 <Text bold>{dirName}</Text>
          {sep} 从此项目
        </Text>;
    }
    ...
    // Multiple read paths
    return <Text>
        是的，允许从此项目读取 {formatPathList(readPaths)}
      </Text>;
  }
  if (hasDirectories && !hasReadPaths && !hasCommands) {
    ...
    if (directories.length === 1) {
      ...
      return <Text>
          是的，始终允许访问 <Text bold>{dirName}</Text>
          {sep} 从此项目
        </Text>;
    }
    ...
    return <Text>
        是的，始终允许从此项目访问 {formatPathList(directories)}
      </Text>;
  }
  ...
  if ((hasDirectories || hasReadPaths) && !hasCommands) {
    ...
    if (hasDirectories && hasReadPaths) {
      ...
      return <Text>
          是的，始终允许从此项目访问 {formatPathList(allPaths)}
        </Text>;
    }
  }
  if ((hasDirectories || hasReadPaths) && hasCommands) {
    ...
    if (allPaths.length === 1 && shellCommands.length === 1) {
      return <Text>
          是的，允许访问 {formatPathList(allPaths)} 和{' '}
          {commandListDisplayTruncated(shellCommands)} 命令
        </Text>;
    }
    return <Text>
        是的，允许 {formatPathList(allPaths)} 访问和{' '}
        {commandListDisplayTruncated(shellCommands)} 命令
      </Text>;
  }
  return null;
