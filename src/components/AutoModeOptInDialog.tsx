...
export const AUTO_MODE_DESCRIPTION = "自动模式让 Claude 自动处理权限提示 \u2014 Claude 会在执行前检查每个工具调用是否存在风险操作和提示注入。Claude 认为安全的操作会被执行，而被识别为有风险的操作会被阻止，Claude 可能会尝试其他方法。适合长时间运行的任务。会话费用会稍高一些。Claude 可能会犯错误导致有害命令被执行，建议仅在隔离环境中使用。Shift+Tab 切换模式。";
type Props = {
  onAccept(): void;
  onDecline(): void;
